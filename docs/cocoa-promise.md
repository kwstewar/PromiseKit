...
* `UIAlertView(/*…*/).promise()`
* `NSURLSession.GET(url).then { (_:Data) in }`
* `NSURLSession.GET(url).asDictionary().then { (json :NSDictionary) in }`
* [View the full listing on GitHub](https://github.com/mxcl/PromiseKit/tree/master/Categories).
